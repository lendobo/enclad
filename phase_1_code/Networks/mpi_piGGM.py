...
def synthetic_sweep_single(param_set):
    p_range, (n, b, Q), lambda_range = param_set
    
    results_dict = {}
    
    # Directly call the function for the given parameter set
    result = synthetic_run(p=p_range, n=n, b=b, Q=Q, lambda_range=lambda_range)
    results = [result]

    key = tuple(list(param_set[:-1]) + [str(param_set[-1])])
    results_dict[key] = {
        'opt_precision_mat': result[0],
        'adj_matrix': result[1],
        'edge_counts_all': result[2],
...
for i in range(1):
    ...
    param_sets = list(product([p_range], zip(n, b_values, Q_values), [lambda_range]))

    # Using SLURM_ARRAY_TASK_ID to determine which parameter set to process
    task_id = int(os.environ.get('SLURM_ARRAY_TASK_ID', 0))  # default to 0 for local testing
    param_set = param_sets[task_id]
    results = synthetic_sweep_single(param_set)
    ...
    with open(f'net_results/results_{p_range,n,b_values,Q_values,len(lambda_range)}__{i}.pkl', 'wb') as f:
...
with open(f'net_results/duration_{p_range,n,b_values,Q_values,len(lambda_range)}.txt', 'w') as f:
